...
CZBEndDevice::CZBEndDevice(const std::string &name, data_vct_ptr _vctAddress)
{
    CZBEndDevice();
    SetName(name);
...
bool CZBEndDevice::searchFrame(data_vct_ptr __vctToSearch)
{
    boost::mutex::scoped_lock scoped_lock(mutex);
...
data_vct_ptr CZBEndDevice::getFrameIDVector(void)
{
    data_vct_ptr vct(new std::vector<boost::uint8_t>());
    std::map<uint8_t,stTXFrame>::iterator it;
    boost::mutex::scoped_lock scoped_lock(mutex);
    for(it=mapTXBuffer.begin();it!=mapTXBuffer.end();++it)
    {
